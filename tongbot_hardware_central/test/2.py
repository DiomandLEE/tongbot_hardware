...
'''
观测量中加入了速度，效果没什么变化，反而有点不好
'''

# 定义卡尔曼滤波器类
class KalmanFilter:
    def __init__(self, dt, process_noise, measurement_noise_position, measurement_noise_velocity):
        self.dt = dt  # 时间步长
        ...
        # 测量矩阵 H (观测位置和速度)
        self.H = np.array([[1, 0, 0],  # 位置测量
                           [0, 1, 0]])  # 速度测量

        # 测量噪声协方差矩阵 R (设置位置和速度的噪声)
        self.R = np.array([[measurement_noise_position, 0],
                           [0, measurement_noise_velocity]])
...
measurement_noise_position = 1e-6  # 位置噪声
measurement_noise_velocity = 5e-5 # 速度噪声（较大）

# 初始化卡尔曼滤波器
kf = KalmanFilter(dt, process_noise, measurement_noise_position, measurement_noise_velocity)
...
for i, t in enumerate(time_steps):
    # 假设我们能同时测量位置和速度，并加入噪声
    measured_pos = true_positions[i] + np.random.normal(0, np.sqrt(measurement_noise_position))
    measured_vel = true_velocities[i] + np.random.normal(0, np.sqrt(0.002))
    measured_positions.append(measured_pos)
    measured_velocities.append(measured_vel)

    # 卡尔曼滤波器的预测和更新步骤
    kf.predict()
    kf.update(np.array([[measured_pos], [measured_vel]]))
...
plt.plot(time_steps[100:], measured_velocities[100:], label="Measured Velocity", color="r", linestyle="--")
...
plt.plot(time_steps[100:], estimated_accelerations[100:], 'r-', label='Estimated Acceleration')
plt.xlabel("Time [s]")
