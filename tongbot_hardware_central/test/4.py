...
'''
这个是使用卡尔曼滤波用到怀疑自我，感觉是不是小车的状态估计也会有这么大的问题
所以对jerk为输入，状态量为x,v,a进行了测试，状态转移方程是正确的
结果发现效果不错，打消了怀疑
'''
...
plt.plot(time_steps[100:], smoothed_accelerations[100:], label="Filtered Acceleration", color="b")
plt.xlabel("Time [s]")
