...
def test_velocity_frames():
    ...
    model = hardware.MobileManipulatorKinematics()
    q = np.array([-1.0, 1.0, 0, 1.5708, -0.7854,
                 1.5708, -0.7854, 1.5708, 1.3100])
...
def test_acceleration_frames():
    ...
    As_a = np.concatenate(
        (model.link_spatial_acceleration(frame="local_world_aligned")))
...
def test_compare_pose_with_pybullet():
    model = hardware.MobileManipulatorKinematics(tool_link_name="gripper")

    pyb.connect(pyb.DIRECT)
    xacro_doc = XacroDoc.from_package_file(
        package_name="tongbot_hardware",
