...
class RobotKinematics:
    ...
    #! 在执行link_pose和link_velocity时，都要调用forward函数，这样才能更新data
    def link_pose(self, link_idx=None, rotation_matrix=False):
        ...
        pose = self.data.oMf[link_idx] # 返回frame在世界坐标系中的位姿
        pos = pose.translation.copy()
        orn = pose.rotation.copy()
        if not rotation_matrix:
            orn = r2q(orn, order="xyzs") # 将旋转矩阵转换为四元数
        return pos, orn
...
class MobileManipulatorKinematics(RobotKinematics): #这是python的继承
    def __init__(self, filepath=None, tool_link_name="gripper"):
        if filepath is None:
            filepath = package_file_path(
                "tongbot_hardware", "urdf/xacro/thing_no_wheels.urdf.xacro"
            ) #todo，需要修改的
        ...
        super().__init__(model, tool_link_name) #调用父类的构造函数
