...
class DataLogger:
    ...
    upright_cmd/scripts/simulations/mpc_sim.py L190
    upright_control/include/upright_control/controller_python_interface.h L72
    参考这个可以，发布 cost的getValue，然后这里接收，存到logg里
    '''

    '''
    生成的npz文件的格式是这样的
    Q_we_ds = np.array([
        [0.7071, 0.0, 0.7071, 0.0],  # 第 1 时间步的目标四元数 (qx, qy, qz, qw)
        [0.7071, 0.0, 0.7071, 0.0],  # 第 2 时间步的目标四元数
    ...
    ])

    Q_wes = np.array([
        [0.7071, 0.0, 0.7071, 0.0],  # 第 1 时间步的实际四元数 (qx, qy, qz, qw)
        [0.7071, 0.0, 0.7071, 0.0],  # 第 2 时间步的实际四元数
    ...
    ])
...
class DataPlotter:
    ...
    def plot_object_error(self, obj_index):
        ...
        for i in range(r_oe_es.shape[0]):
            C_ew = tmath.quat_to_rot(tmath.quat_inverse(Q_wes[i, :]))
        ...
        Q_ow0 = tmath.quat_inverse(Q_wos[0, :])
        Q_oe0 = tmath.quat_multiply(Q_ow0, Q_wes[0, :])
        Q_eo_err = np.zeros_like(Q_wos)
        angles = np.zeros(Q_wos.shape[0])
        for i in range(Q_eo_err.shape[0]):
            try:
                Q_ew = tmath.quat_inverse(Q_wes[i, :])
                Q_eo = tmath.quat_multiply(Q_ew, Q_wos[i, :])
                Q_eo_err[i, :] = tmath.quat_multiply(Q_oe0, Q_eo)
                angles[i] = tmath.quat_angle(Q_eo_err[i, :])
