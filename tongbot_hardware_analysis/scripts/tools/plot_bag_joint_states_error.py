...
def read_csv_and_separate(ref_csv_file):
    # 读取CSV文件
    data = pd.read_csv(ref_csv_file, header=None)  # 假设没有列名

    # 将时间列（第一列）提取出来
    time = data.iloc[:, 0].values  # 获取第一列作为时间
    # 将其余的列作为状态（state）
    state = data.iloc[:, 1:].values  # 获取第2到最后一列作为状态
...
def main():
    # 示例使用
    ref_csv_file = "your_file.csv"  # 请替换为 跟踪的Reference CSV文件路径
    time_array, state_q_ref = read_csv_and_separate(ref_csv_file)

    # print("Time array:", time_array)
    # print("State array:", state_q_ref)
    ...
    分别读取 kinova/joint_states（Dingo 机器人的关节状态）、dingo/joint_states（Ridgeback 基座的关节状态）、
    ...
    #?  由于ref中，time_array本身就是从0开始，所以这里根据归一化后的t0进行插值
    state_q_ref_aligned = ros_utils.interpolate_list(ts, time_array, state_q_ref)
    ...
    plt.figure()
    for i in range(n):
        plt.plot(ts, state_q_ref_aligned[:, i], label=f"$q_{i+1}$")
    plt.title("Reference Joint Positions")
    plt.xlabel("Time (s)")
    ...
    plt.figure()
    for i in range(n):
        plt.plot(ts, qs_real[:, i] - state_q_ref_aligned[:, i], label=f"$q_{i+1}$")
    plt.title("Reference Joint Position Error: Real - Reference")
    plt.xlabel("Time (s)")
    plt.ylabel("Joint position")
    plt.legend()
    plt.grid()
