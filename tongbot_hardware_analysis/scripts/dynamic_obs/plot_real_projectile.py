...
VICON_DYNAMIC_NAME = "Dynamic_Obs"
...
def main():
    ...
    est_msgs = [msg for _, msg, _ in bag.read_messages("/dynamic_obs/joint_states")]
    est_positions = np.array([msg.position for msg in est_msgs])
    est_velocities = np.array([msg.velocity for msg in est_msgs])

    vicon_topic = ros_utils.vicon_topic_name(VICON_DYNAMIC_NAME)
    ...
    #! 这一段是找出卡尔曼滤波开始滤的时间，因为在upright中，是有一个active_flight_height的参数，当高度大于这个值的时候，才开始考虑障碍物。
    nv = vicon_positions.shape[0]
    # for i in range(nv):
    #     if vicon_positions[i, 2] >= 0.8:
    #         active_time = vicon_times[i]
    #         break
    # 对于本实验而言，active_time就是vicon_times的开始点
    active_time = vicon_times[0]
